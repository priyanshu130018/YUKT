  ...
  return (
    <ToolLayout toolId="image-resizer">
      <div className="flex-1 flex flex-col items-center p-8 max-w-4xl mx-auto w-full">
        <h2 className="text-[#1a1a1a] dark:text-white text-3xl font-black mb-8 uppercase tracking-tight text-center">Image Resizer</h2>
        ...
        {!img ? (
          <label onClick={()=>fileRef.current?.click()} className="w-full h-64 border-2 border-dashed border-gray-200 dark:border-[#ffffff10] bg-white dark:bg-zinc-900/40 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-[#1a1a1a] dark:hover:border-white hover:bg-gray-50 dark:hover:bg-white/5 transition-all group">
            ...
            <p className="text-[#1a1a1a] dark:text-white font-black uppercase tracking-widest text-sm">Upload Image to Resize</p>
          ...
          <div className="w-full space-y-8">
            <div className="bg-white dark:bg-zinc-900/40 rounded-2xl p-6 border border-gray-100 dark:border-[#ffffff10] shadow-sm flex flex-col items-center">
              <div className="relative group overflow-hidden rounded-xl border border-gray-100 dark:border-[#ffffff05] mb-4 bg-gray-50 dark:bg-[#1a1a1a]/40 p-2">
                <img src={img} alt="Original" className="max-h-64 rounded-lg object-contain shadow-sm"/>
                <div className="absolute inset-0 bg-[#1a1a1a]/5 opacity-0 group-hover:opacity-100 transition-all"/>
              </div>
            ...
            <div className="bg-white dark:bg-zinc-900/40 rounded-2xl p-8 border border-gray-100 dark:border-[#ffffff10] shadow-lg">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end">
                <div className="space-y-2">
                  ...
                  <input type="number" value={w} onChange={e=>onW(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-[#ffffff10] text-[#1a1a1a] dark:text-white font-black focus:ring-1 focus:ring-[#1a1a1a] dark:focus:ring-white outline-none"/>
                </div>
                <div className="flex flex-col items-center pb-3">
                  <button onClick={()=>setLock(!lock)} className={`p-2 rounded-lg transition-all shadow-sm ${lock?'bg-[#1a1a1a] text-white dark:bg-white dark:text-[#1a1a1a]':'bg-gray-100 text-gray-400 dark:bg-white/5'}`}>
                    {lock ? '🔒' : '🔓'}
                  </button>
                ...
                <div className="space-y-2">
                  ...
                  <input type="number" value={h} onChange={e=>onH(e.target.value)} className="w-full px-4 py-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-[#ffffff10] text-[#1a1a1a] dark:text-white font-black focus:ring-1 focus:ring-[#1a1a1a] dark:focus:ring-white outline-none"/>
                </div>
              </div>
              <button onClick={resize} className="w-full mt-8 py-4 rounded-xl text-white dark:text-[#1a1a1a] font-black uppercase tracking-widest bg-[#1a1a1a] dark:bg-white hover:bg-gray-900 dark:hover:bg-gray-200 transition-all shadow-lg active:scale-[0.98]">Process Resizing</button>
            </div>
          </div>
        )}

        {result && (
          <div className="w-full mt-8 bg-[#1a1a1a] dark:bg-white rounded-2xl p-10 flex flex-col items-center animate-in fade-in slide-in-from-bottom-5 shadow-2xl relative overflow-hidden group">
            ...
            <div className="mb-6 p-2 bg-white/10 dark:bg-[#1a1a1a]/5 rounded-xl backdrop-blur-sm border border-white/5 dark:border-[#1a1a1a]/5 shadow-2xl">
              <img src={result} alt="Resized Result" className="max-h-64 rounded-lg shadow-sm border border-white/10 dark:border-[#1a1a1a]/10"/>
            </div>
            <div className="flex flex-col items-center gap-4">
              <div className="text-center">
                <p className="text-white dark:text-[#1a1a1a] font-black uppercase tracking-widest text-lg">Resizing Complete</p>
                <p className="text-white/60 dark:text-[#1a1a1a]/60 text-xs font-bold uppercase mt-1">Output Size: {w} × {h}px</p>
              </div>
              <a href={result} download={`yukt_resized_${w}x${h}.png`} className="flex items-center gap-3 px-10 py-4 rounded-xl bg-white dark:bg-[#1a1a1a] text-[#1a1a1a] dark:text-white font-black uppercase tracking-widest shadow-xl hover:bg-gray-50 dark:hover:bg-zinc-900 transition-all active:scale-95">
