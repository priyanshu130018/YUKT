  ...
  return (
    <ToolLayout toolId="image-to-pdf">
      <div className="flex-1 flex flex-col items-center p-8 max-w-2xl mx-auto w-full">
        <h2 className="text-black dark:text-white text-3xl font-black mb-10 uppercase tracking-tight text-center">Images to PDF Architect</h2>
        <label onClick={()=>fileRef.current?.click()} className="w-full border-2 border-dashed border-gray-200 dark:border-[#ffffff10] bg-white dark:bg-zinc-900/40 rounded-2xl p-12 text-center cursor-pointer hover:border-[black] dark:hover:border-white hover:bg-gray-50 dark:hover:bg-white/5 mb-8 transition-all group shadow-sm">
          ...
          <p className="text-black dark:text-white font-black uppercase tracking-widest text-sm">Upload images (each = 1 PDF page)</p>
        ...
        {images.length>0&&<div className="w-full grid grid-cols-4 sm:grid-cols-6 gap-3 mb-8">
          {images.map((i,idx)=><div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-gray-100 dark:border-[#ffffff05] shadow-sm">
            ...
            <span className="absolute top-1 left-1 bg-[black]/80 dark:bg-white/80 text-white dark:text-black text-[8px] font-black px-1.5 py-0.5 rounded-md shadow-lg">{String(idx+1).padStart(2,'0')}</span>
          </div>)}
        </div>}
        <div className="flex gap-4 w-full">
          <button onClick={convert} disabled={!images.length||loading} className="flex-1 py-4 rounded-xl text-white dark:text-black font-black uppercase tracking-widest bg-[black] dark:bg-white hover:bg-gray-900 dark:hover:bg-gray-200 transition-all shadow-xl disabled:opacity-50 active:scale-95">
