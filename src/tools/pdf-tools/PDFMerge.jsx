  ...
  return (
    <ToolLayout toolId="pdf-merger">
      <div className="flex-1 flex flex-col items-center p-8 max-w-2xl mx-auto w-full">
        <h2 className="text-black dark:text-white text-3xl font-black mb-10 uppercase tracking-tight text-center">PDF Sequential Merger</h2>
        <label onClick={()=>fileRef.current?.click()} className="w-full border-2 border-dashed border-gray-200 dark:border-[#ffffff10] bg-white dark:bg-zinc-900/40 rounded-2xl p-12 text-center cursor-pointer hover:border-[black] dark:hover:border-white hover:bg-gray-50 dark:hover:bg-white/5 mb-8 transition-all group shadow-sm">
          ...
          <p className="text-black dark:text-white font-black uppercase tracking-widest text-sm">Add PDF documents to merge</p>
        ...
        {files.map((f,i)=>(
          <div key={i} className="w-full flex items-center justify-between gap-4 px-6 py-4 bg-white dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-[#ffffff10] mb-3 transition-all group hover:border-[black]/10 dark:hover:border-white/10">
            <div className="flex items-center gap-3">
              <span className="text-gray-300 dark:text-zinc-600 text-[10px] font-black w-6">#{String(i+1).padStart(2,'0')}</span>
              <span className="text-black dark:text-white text-sm font-black truncate">{f.name}</span>
            </div>
        ...
        <button onClick={merge} disabled={files.length<2||loading} className="w-full py-4 rounded-xl text-white dark:text-black font-black uppercase tracking-widest bg-[black] dark:bg-white hover:bg-gray-900 dark:hover:bg-gray-200 transition-all shadow-xl disabled:opacity-50 active:scale-95 mt-6">
