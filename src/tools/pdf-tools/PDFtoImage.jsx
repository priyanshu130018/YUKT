  ...
  return (
    <ToolLayout toolId="pdf-to-image">
      <div className="flex-1 flex flex-col items-center p-8 max-w-2xl mx-auto w-full">
        <h2 className="text-black dark:text-white text-3xl font-black mb-2 uppercase tracking-tight text-center">PDF to Image Converter</h2>
        ...
        <label onClick={()=>fileRef.current?.click()} className="w-full border-2 border-dashed border-gray-200 dark:border-[#ffffff10] bg-white dark:bg-zinc-900/40 rounded-2xl p-12 text-center cursor-pointer hover:border-[black] dark:hover:border-white hover:bg-gray-50 dark:hover:bg-white/5 mb-10 transition-all group shadow-sm">
          ...
          <p className="text-black dark:text-white font-black uppercase tracking-widest text-sm">{loading?'Executing Extraction...':'Upload PDF Document'}</p>
        ...
        {pages.length>0&&<div className="w-full grid sm:grid-cols-2 gap-6">
          {pages.map(({url,page})=>(
            <div key={page} className="bg-white dark:bg-zinc-900/40 rounded-2xl overflow-hidden border border-gray-100 dark:border-[#ffffff10] shadow-xl group hover:border-[black]/10 dark:hover:border-white/10 transition-all">
              <div className="p-3 bg-gray-50 dark:bg-black/40 border-b border-gray-100 dark:border-[#ffffff05]">
              ...
              <div className="flex items-center justify-between px-6 py-4">
                ...
                <a href={url} download={`page_${page}.png`} className="px-6 py-2 rounded-xl bg-[black] dark:bg-white text-white dark:text-black text-[10px] font-black uppercase tracking-widest hover:bg-gray-900 dark:hover:bg-gray-200 transition-all shadow-md active:scale-95">
                  Export PNG
                </a>
              </div>
            </div>
