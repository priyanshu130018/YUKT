    ...
    <ToolLayout toolId="age-calculator">
      <div className="flex-1 flex items-center justify-center p-8 max-w-4xl mx-auto w-full">
        <div className="w-full">
          <h2 className="text-black dark:text-white text-3xl font-black mb-8 uppercase tracking-tight text-center">Age Processor</h2>
          ...
          <div className="bg-white dark:bg-zinc-900/40 rounded-2xl p-8 border border-gray-100 dark:border-[#ffffff10] shadow-lg flex flex-col sm:flex-row gap-4 mb-10 transition-colors">
            <div className="flex-1 space-y-2">
              ...
              <input type="date" value={dob} max={new Date().toISOString().split('T')[0]} 
                ...
                className="w-full px-5 py-3.5 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-[#ffffff10] text-black dark:text-white font-black focus:ring-2 focus:ring-[black]/10 dark:focus:ring-white/10 outline-none transition-all" />
            </div>
            <button onClick={calculate} className="sm:mt-6 px-10 py-3.5 rounded-xl text-white dark:text-black font-black uppercase tracking-widest bg-[black] dark:bg-white hover:bg-gray-900 dark:hover:bg-gray-200 transition-all shadow-lg active:scale-95">Analyze Age</button>
          </div>

          {result && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="bg-[black] dark:bg-white rounded-3xl p-10 text-center shadow-2xl relative overflow-hidden group">
                ...
                <p className="text-white dark:text-black text-[10px] font-black uppercase tracking-[0.4em] mb-4 opacity-60">Calculated Biological Age</p>
                <div className="text-white dark:text-black text-5xl md:text-6xl font-black tracking-tighter flex items-baseline justify-center gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                {[
                  ...
                  <div key={l} className="bg-white dark:bg-white/5 rounded-2xl p-6 text-center border border-gray-100 dark:border-[#ffffff10] shadow-sm hover:border-[black]/20 dark:hover:border-white/40 transition-all group">
                    <p className="text-gray-400 text-[8px] font-black uppercase tracking-widest mb-1 group-hover:text-black dark:text-white transition-colors">{slug}</p>
                    <p className="text-black dark:text-white font-black text-2xl tracking-tighter">{v.toLocaleString()}</p>
