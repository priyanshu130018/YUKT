      ...
      <div className="flex-1 flex flex-col items-center justify-center p-8 max-w-4xl mx-auto w-full">
          ...
          <div className="flex items-center gap-6 mb-12 animate-in fade-in slide-in-from-top-4">
            {[['Minutes', inputMin, setInputMin, 99, v => setRemaining(+v*60+inputSec)],
              ...
              <div key={label} className="text-center group">
                <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-2 group-hover:text-[#1a1a1a] dark:text-white transition-colors">{label}</p>
                <input type="number" min="0" max={max} value={val}
                  onChange={e => { set(+e.target.value); extra(e.target.value); }}
                  className="w-24 text-center text-[#1a1a1a] dark:text-white bg-white dark:bg-white/5 border border-gray-100 dark:border-[#ffffff10] rounded-2xl py-4 text-3xl font-black shadow-sm focus:ring-2 focus:ring-[#1a1a1a]/20 dark:focus:ring-white/10 outline-none transition-all" />
              </div>
            ))}
          </div>
        )}
        ...
        <div className="relative w-80 h-80 mx-auto mb-12">
          <svg className="w-full h-full -rotate-90 drop-shadow-2xl" viewBox="0 0 220 220">
            ...
            <circle cx="110" cy="110" r="100" fill="none" stroke="currentColor" className="text-[#1a1a1a] dark:text-white" strokeWidth="12" strokeLinecap="round"
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            {finished ? (
              <div className="text-center animate-bounce">
                <p className="text-[#1a1a1a] dark:text-white text-4xl font-black uppercase tracking-tighter">Done</p>
                <p className="text-gray-400 text-xs font-bold mt-1">Session Ended</p>
              </div>
            ) : (
              <div className="text-center">
                <p className="text-[#1a1a1a] dark:text-white text-6xl font-black tabular-nums tracking-tighter leading-none">{pad(Math.floor(remaining/60))}:{pad(remaining%60)}</p>
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={reset} className="p-5 rounded-2xl bg-white dark:bg-white/5 border border-gray-100 dark:border-[#ffffff10] text-gray-400 hover:text-[#1a1a1a] dark:text-white hover:border-[#1a1a1a] dark:hover:border-white transition-all shadow-sm group">
