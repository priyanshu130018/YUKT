    ...
    <ToolLayout toolId="clock">
      <div className="flex-1 flex flex-col items-center justify-center p-8">
        <div className="bg-white dark:bg-white/5 rounded-3xl px-16 py-12 border border-gray-100 dark:border-[#ffffff10] mb-12 text-center shadow-xl transition-colors" style={{ boxShadow: '0 20px 50px rgba(0,0,0,0.05)' }}>
          <div className="text-8xl md:text-9xl font-black tracking-tighter tabular-nums text-black dark:text-white">
            {pad(h12)}:{pad(m)}:{pad(s)}
          </div>
          <div className="flex items-center justify-center gap-3 mt-4">
            <div className="px-3 py-1 bg-[black] dark:bg-white dark:text-black text-white text-xs font-black rounded uppercase tracking-widest">{ampm}</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 w-full max-w-4xl">
          {zones.map(z => (
            <div key={z.name} className="bg-white dark:bg-white/5 rounded-2xl p-5 border border-gray-50 dark:border-[#ffffff10] text-center shadow-sm hover:border-[black]/20 dark:hover:border-white/40 transition-all">
              <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-2">{z.name}</p>
              <p className="text-black dark:text-white font-black text-lg tabular-nums">{new Date().toLocaleTimeString('en-US', { timeZone: z.tz, hour: '2-digit', minute: '2-digit', hour12: true })}</p>
            </div>
          ))}
        </div>
      </div>
