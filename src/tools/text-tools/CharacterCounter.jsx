  ...
  return (
    <ToolLayout toolId="character-counter">
      <div className="flex-1 flex flex-col p-8 max-w-4xl mx-auto w-full">
        <h2 className="text-black dark:text-white text-3xl font-black mb-6 uppercase tracking-tight text-center">Character Counter</h2>
        <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="Type text to see character frequency..."
          className="min-h-48 p-5 rounded-2xl bg-white dark:bg-zinc-900/40 border border-gray-200 dark:border-[#ffffff10] text-black dark:text-white text-lg placeholder-gray-400 focus:ring-2 focus:ring-[black]/10 dark:focus:ring-white/10 outline-none transition-all resize-none mb-6 shadow-inner"/>
        ...
        {sorted.length > 0 && (
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            <h3 className="text-black dark:text-white font-black text-sm mb-4 uppercase tracking-widest">Frequency Analysis</h3>
            <div className="space-y-3">
              {sorted.map(([c,n])=>(
                <div key={c} className="flex items-center gap-4">
                  <span className="text-black dark:text-white font-mono font-black w-8 h-8 flex items-center justify-center bg-gray-50 dark:bg-white/5 rounded-lg border border-gray-100 dark:border-[#ffffff05]">{c==' '?'·':c}</span>
